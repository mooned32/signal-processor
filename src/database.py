...
def init_database(db_path: Path) -> None:
    try:
        ...
        raise DatabaseError(
            f"Не удалось инициализировать базу данных '{db_path.name}': {err}"
        ) from err
    ...
    try:
        with sqlite3.connect(db_path) as connection:
            ...
            if measurement_id is None:
                raise DatabaseError("Не удалось получить идентификатор созданной записи.")
        ...
        raise DatabaseError(f"Ошибка сохранения измерения в базу данных: {err}") from err
